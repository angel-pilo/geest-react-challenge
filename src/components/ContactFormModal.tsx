...
const inputClass =
  'mt-2 w-full rounded-lg border bg-white px-3.5 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-brand focus:outline-2 focus:outline-offset-1 focus:outline-brand/20'
...
export function ContactFormModal({ onClose, onAdd }: Props) {
  ...
  function visibleError(field: keyof ContactFormValues) {
    return formik.touched[field] || formik.values[field] !== ''
      ? formik.errors[field]
      : undefined
  }
  const requiredEmpty =
    ...
    >
      <div className="p-6 sm:p-8">
        <div className="mb-5 flex items-start justify-between">
          <span className="rounded-xl bg-[#edf4ef] p-3 text-brand">
        ...
        >
          {(
            ...
            ({ field, label, type, placeholder, autocomplete, required }) => {
              ...
              return (
                <div key={field}>
                  ...
                  <input
                    ...
                    className={`${inputClass} ${error ? 'border-red-500' : 'border-slate-300'}`}
                  />
                  {error && (
                    <p
                      id={`${field}-error`}
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) => {
                void formik.setFieldTouched('department', true, false)
                formik.handleChange(event)
              }}
              aria-invalid={Boolean(visibleError('department'))}
              aria-describedby={
                visibleError('department') ? 'department-error' : undefined
              }
              className={`${inputClass} ${visibleError('department') ? 'border-red-500' : 'border-slate-300'}`}
            >
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-6">
            ...
            <button
              type="submit"
              disabled={!formik.isValid || requiredEmpty || formik.isSubmitting}
              className="rounded-lg bg-brand px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-900 disabled:bg-slate-200 disabled:text-slate-500"
            >
              {formik.isSubmitting ? 'Guardando…' : 'Guardar contacto'}
            </button>
          </div>
        </form>
      </div>
