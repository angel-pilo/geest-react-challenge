...
it('almacena el UUID generado y conserva el teléfono opcional', async () => {
  ...
  act(() => result.current.addContact({ name: 'Laura', email: 'laura@example.com', department: 'Soporte' }))
  expect(result.current.contacts).toEqual([{ id, name: 'Laura', email: 'laura@example.com', department: 'Soporte' }])
})
